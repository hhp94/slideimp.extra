...
ilmn_meth_mani <- dplyr::mutate(
  ilmn_meth_mani,
  dplyr::across(.cols = dplyr::everything(), .fns = \(x) {
    purrr::set_names(x, chip)
  })
)
...
EPICv2dd_excl <- EPICv2dd[
  feature %in% EPICv2dd[, .N, by = feature][N > 1, feature]
][group == "0", ]
EPICv2dd_excl
...
dcast(
  MSAdd[feature %in% MSAdd[, .N, by = feature][N > 1, feature]],
  feature ~ group
)
all(
  dcast(
...
MSAdd_excl <- MSAdd[feature %in% MSAdd[, .N, by = feature][N > 1, feature]][
  group == "0",
]
MSAdd_excl
